...
const MODEL_CONFIG = {
  vocabSize: 10000,          // Top 10K words (↑ from 5K) - richer vocabulary
  embeddingDim: 128,         // Medium embeddings (↑ from 64) - richer vectors
  hiddenDim: 256,            // Larger hidden layer (↑ from 128) - deeper learning
  numClasses: {              // Multi-task classification
    // Original outputs
    sourceType: 5,           // website, academic, book, news, arxiv
    hasAuthor: 2,            // binary: yes/no
    citationStyle: 3,        // MLA, APA, Chicago
    // Tier 1 outputs (new)
    isCorporateAuthor: 2,    // binary: organization vs person
    numAuthors: 3,           // 1, 2-3, 4+ (for et al. rules)
    isPeerReviewed: 2,       // binary: scholarly vs non-scholarly
    hasDate: 2,              // binary: date present vs missing (for n.d.)
    isSocialMedia: 2,        // binary: Twitter/Reddit/etc vs other
  },
  maxLength: 128,            // Max sequence length
  dropoutRate: 0.3,          // Higher dropout (↑ from 0.1) - better regularization
};
...
function createCitationModel(config = MODEL_CONFIG) {
  ...
  // 1. Source type classification (5 classes)
  ...
  // ========== TIER 1 OUTPUT HEADS (NEW) ==========

  // 4. Is Corporate Author (binary: organization vs person)
  const corporateAuthorOutput = tf.layers.dense({
    units: config.numClasses.isCorporateAuthor,
    activation: 'softmax',
    name: 'is_corporate_author_output'
  }).apply(x);

  // 5. Number of Authors (3 classes: 1, 2-3, 4+)
  const numAuthorsOutput = tf.layers.dense({
    units: config.numClasses.numAuthors,
    activation: 'softmax',
    name: 'num_authors_output'
  }).apply(x);

  // 6. Is Peer-Reviewed (binary: scholarly vs non-scholarly)
  const peerReviewedOutput = tf.layers.dense({
    units: config.numClasses.isPeerReviewed,
    activation: 'softmax',
    name: 'is_peer_reviewed_output'
  }).apply(x);

  // 7. Has Date (binary: date present vs missing)
  const hasDateOutput = tf.layers.dense({
    units: config.numClasses.hasDate,
    activation: 'softmax',
    name: 'has_date_output'
  }).apply(x);

  // 8. Is Social Media (binary: Twitter/Reddit/etc vs other)
  const socialMediaOutput = tf.layers.dense({
    units: config.numClasses.isSocialMedia,
    activation: 'softmax',
    name: 'is_social_media_output'
  }).apply(x);

  // Create model with all 8 outputs
  const model = tf.model({
    ...
    outputs: [
      ...
      authorOutput,            // 1
      styleOutput,             // 2
      corporateAuthorOutput,   // 3
      numAuthorsOutput,        // 4
      peerReviewedOutput,      // 5
      hasDateOutput,           // 6
      socialMediaOutput        // 7
    ],
    name: 'citation_model'
  ...
  // Compile model with loss functions for all outputs
  model.compile({
    optimizer: tf.train.adam(0.001),
    loss: {
      source_type_output: 'sparseCategoricalCrossentropy',
      has_author_output: 'sparseCategoricalCrossentropy',
      citation_style_output: 'sparseCategoricalCrossentropy',
      is_corporate_author_output: 'sparseCategoricalCrossentropy',
      num_authors_output: 'sparseCategoricalCrossentropy',
      is_peer_reviewed_output: 'sparseCategoricalCrossentropy',
      has_date_output: 'sparseCategoricalCrossentropy',
      is_social_media_output: 'sparseCategoricalCrossentropy',
    },
    lossWeights: {
      source_type_output: 1.0,
      has_author_output: 0.5,
      citation_style_output: 0.8,
      is_corporate_author_output: 0.6,
      num_authors_output: 0.6,
      is_peer_reviewed_output: 0.7,
      has_date_output: 0.5,
      is_social_media_output: 0.6,
    },
    metrics: {
      source_type_output: 'accuracy',
      has_author_output: 'accuracy',
      citation_style_output: 'accuracy',
      is_corporate_author_output: 'accuracy',
      num_authors_output: 'accuracy',
      is_peer_reviewed_output: 'accuracy',
      has_date_output: 'accuracy',
      is_social_media_output: 'accuracy',
    },
  });
